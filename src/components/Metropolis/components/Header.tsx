...
const Header = () => {
  ...
  return (
    <div>
      <header
        className={cn(
          scrolled ? "bg-[#131C25]" : "bg-transparent",
          "flex justify-center px-5 py-5 fixed z-50 top-0 left-0 right-0 transition-all duration-500"
        )}
      >
        <div className="xl:max-w-6xl w-full flex items-center justify-between">
          ...
          <div className="items-center space-x-4 ml-8 overflow-auto hidden md:flex">
            {LINKS.map((link) => {
              return (
                <a
                  ...
                  key={link.label}
                  target="_blank"
                  className="text-sm font-semibold px-3 text-white transition-colors duration-200 ease-in hover:no-underline hover:text-[#c7f284]"
                >
                  {link.label}
                </a>
              );
            ...
            <a
              ...
              target="_blank"
              className={cn(
                "text-sm font-semibold text-center text-[#c7f284]",
                "bg-[#4A5C41]/90 py-[10px] px-[15px]",
                "border border-solid border-transparent rounded-full",
                "transition-colors duration-200 ease-out hover:no-underline hover:text-[#c7f284] hover:border-[#c7f284]"
              )}
            >
              Start Building
            </a>
      ...
      <div className="fixed top-[20px] right-7 z-50 block md:hidden">
        <Menu
          ...
          customBurgerIcon={
            <BurgerIcon width={30} height={30} color={"#ffffff"} />
          }
          customCrossIcon={
        ...
        >
          <div className="w-full">
            <div
              className={cn(
                "w-full h-screen flex flex-col overflow-auto gap-2 items-center pt-6",
                "bg-gradient-to-b from-[#223345] to-[#131C25]",
            ...
            >
              ...
              {LINKS.map((link) => {
                return (
                  <a
                    ...
                    key={link.label}
                    target="_blank"
                    className="text-sm font-semibold text-white py-1 transition-colors duration-200 ease-in hover:no-underline hover:text-[#c7f284]"
                  >
                    {link.label}
                  </a>
                );
              ...
              <a
                ...
                className={cn(
                  "text-sm font-semibold text-center text-[#c7f284]",
                  "border border-solid border-transparent rounded-full",
                  "bg-[#4A5C41]/90 py-[10px] px-4 mt-4",
                  "transition-colors duration-200 ease-out hover:no-underline hover:text-[#c7f284] hover:border-[#c7f284]"
                )}
              >
                Start Building
